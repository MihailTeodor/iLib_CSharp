...
namespace iLib.src.main.services
{
    ...
    [ApiController]
    public class ArticleEndpoint(IArticleController articleController) : ControllerBase
    {
        private readonly IArticleController _articleController = articleController;
        ...
        public IActionResult CreateArticle([FromBody] ArticleDTO articleDTO)
        {
            try
            {
                Guid id = _articleController.AddArticle(articleDTO);
        ...
        public IActionResult UpdateArticle(Guid articleId, [FromBody] ArticleDTO articleDTO)
        {
            try
            {
                _articleController.UpdateArticle(articleId, articleDTO);
        ...
        public IActionResult GetArticleInfo(Guid articleId)
        {
            try
            {
                var articleDTO = _articleController.GetArticleInfoExtended(articleId);
        ...
        public IActionResult SearchArticles([FromQuery] string? isbn, [FromQuery] string? issn, [FromQuery] string? isan, [FromQuery] string? title, [FromQuery] string? genre, [FromQuery] string? publisher, [FromQuery] string? yearEdition, [FromQuery] string? author, [FromQuery] int? issueNumber, [FromQuery] string? director, [FromQuery] int pageNumber = 1, [FromQuery] int resultsPerPage = 10)
        {
            ...
            try
            {
                long totalResults = _articleController.CountArticles(isbn, issn, isan, title, genre, publisher, _yearEdition, author, issueNumber, director);
                ...
                var articleDTOs = _articleController.SearchArticles(isbn, issn, isan, title, genre, publisher, _yearEdition, author, issueNumber, director, fromIndex, resultsPerPage)
        ...
        public IActionResult DeleteArticle(Guid articleId)
        {
            try
            {
                _articleController.RemoveArticle(articleId);
